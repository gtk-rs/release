    ...
    if res.status_code != 200:
        if res.status_code == 403:
            ...
            if (
                ...
                and "X-RateLimit-Remaining" in res.headers
                and "X-RateLimit-Reset" in res.headers
            ):
...
def get_all_contents(
    url, state=None, max_date=None, token=None, recursive=True, params=None
):
    if params is None:
        params = {}
    headers = {
    ...
    while next_page <= last_page:
        res = get_url_data(
            next_page_url.replace(f"&{to_replace}", f"&page={next_page}"), headers, None
        )
        if res.status_code != 200:
            break
        content = res.json()
...
class Github:
    ...
    def get_pulls(self, repo_name, repo_owner, state, max_date, only_merged=False):
        return Repository(self, repo_name, repo_owner).get_pulls(
            state, max_date, only_merged=only_merged
        )
...
class Organization:
    ...
    def get_repositories(self):
        repos = get_all_contents(
            f"https://api.github.com/orgs/{self.name}/repos", token=self.gh_obj.token
        )
        if repos is None:
            return []
        return [
            Repository(self.gh_obj, repo["name"], repo["owner"]["login"])
            for repo in repos
        ]
...
class Repository:
    ...
    def get_pulls(self, state, max_date, only_merged=False):
        prs = get_all_contents(
            f"https://api.github.com/repos/{self.owner}/{self.name}/pulls",
            state,
            max_date,
        ...
        if prs is None:
            return []
        return [
            self.create_pr_obj(pull_request)
            for pull_request in prs
            if (
                only_merged is False
                or (
                    pull_request["merged_at"] is not None
                    and len(pull_request["merged_at"]) > 0
                )
            )
        ]
